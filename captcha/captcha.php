...
class CAPTCHA{	
	function __construct(){
		...
		do{
			...
			$font_file=$fonts[random_int(0, count($fonts)-1)];
			...
			for($i=0;$i<$length;$i++){
				...
				$y=(($i%2)*$fluctuation_amplitude - $fluctuation_amplitude/2)*$odd
					+ random_int(-round($fluctuation_amplitude/3), round($fluctuation_amplitude/3))
		...
		for($i=0;$i<(($height-30)*$x)*$white_noise_density;$i++){
			imagesetpixel($img, random_int(0, $x-1), random_int(10, $height-15), $white);
		}
		for($i=0;$i<(($height-30)*$x)*$black_noise_density;$i++){
			imagesetpixel($img, random_int(0, $x-1), random_int(10, $height-15), $black);
		}
